namespace DynamicLight2D
{
	...
	*/
		...
		static void addOccluder_urp()
		{
			reloadAsset();
			Material m = AssetDatabase.LoadAssetAtPath(_path + "Prefabs/Lights/Materials/StandardOccluderMaterial_URP.mat", typeof(Material)) as Material;
			GameObject gameObject = new GameObject("2DDLight");
			gameObject.transform.position = getCameraPos();
			DynamicLight2D.DynamicLight s = gameObject.AddComponent<DynamicLight2D.DynamicLight>();
			s.Layer.value = AssetUtility.LoadPropertyAsInt("layerMask", tmpAsset);
	...
	*/
		...
		static void addSquare3_urp()
		{
			reloadAsset();
			Object prefab = AssetDatabase.LoadAssetAtPath(_path + "Prefabs/Casters/box.prefab", typeof(GameObject));
			Material m = AssetDatabase.LoadAssetAtPath(_path + "Prefabs/Casters/Materials/SpriteOccluded_URP.mat", typeof(Material)) as Material;

			GameObject hex = Instantiate(prefab, Vector3.zero, Quaternion.identity) as GameObject;
			hex.GetComponent<BoxCollider2D>().enabled = false;
			OccludedUR_passToShader _script = hex.AddComponent<OccludedUR_passToShader>();
			_script.Light2D = Transform.FindObjectOfType<DynamicLight>();
